...
| Decks and study            | Store user content and schedule study. Proposed basis: contract, Art. 6(1)(b).                                                 | Deck/card free text, filenames, categories, review history, scheduling/progress, sharing preference, subject prerequisite graphs (concept names, edges with reasons) and card-to-concept links, and lesson content (modules, lessons, numbered screens with their revision history and whether an AI wrote them, questions and their answers, lesson prerequisites), and the learner's own lesson history (where they are in each lesson, every answer they gave with its time, the review schedule of each question, and the comments the owner leaves on screens while learning). | IONOS VPS in Frankfurt; encrypted Android local database.                    | Account lifetime; individual deck or full account deletion; backups expire within three months.                                                                               | Authenticated ownership checks; encrypted Android storage.                        |
